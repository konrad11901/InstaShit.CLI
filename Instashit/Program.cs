...
namespace Instashit
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            //Inicjowanie sesji/sprawdzanie czy już takowa istnieje
            ...
            //Liczniki: wyrazów, błędów w danej serii, oraz indeks obecnej serii
            ...
            while(true)
            {
                ...
                JSONResponse = JsonConvert.DeserializeObject<Dictionary<string, object>>(await result.Content.ReadAsStringAsync());
                //Sprawdzanie na wszelki wypadek, czy wszystko jest OK
            ...
            foreach (var key in sessionCount.Keys.ToList())
                if (sessionCount[key] != -1)
                    sessionCount[key]++;
            File.WriteAllText(Path.Combine(assemblyLocation, "wordsHistory.json"), JsonConvert.SerializeObject(sessionCount));
        ...
        static Settings GetSettings()
        {
            ...
            Console.Write("Save this settings (y/n)? ");
            if (CanContinue())
                File.WriteAllText(Path.Combine(assemblyLocation, "settings.json"), JsonConvert.SerializeObject(settings));
            return settings;
        }
